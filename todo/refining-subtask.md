...
The subtask refinement workflow is a systematic approach to validating and refining a subtask. It is designed to be prepare the task for highly efficient and effective implementation. Suitable for a simple AI coding agent or junior developer. The process aim to validate the subtask and ensure that it is well defined, with examples and references and that the implementation details are sufficient and builds upon the previous subtask and next subtask as well as the project as a whole.

## Variables

- `<subtaskId>`: The id of the subtask you are validating and refining
...
- No assumptions of correctness or completeness are to be made of the subtask, the documentation or the project. If something is unclear, seem wrong or does not make total sense, it is likely that it is not correct or complete and needs to be redefined.
-
