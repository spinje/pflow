...
11. Cache lint — warns when shell nodes have no template inputs and no explicit `cache:` setting (stale cache risk); suggests both `cache: false` (live state) and `cache: true` (confirm static, silences warning)
