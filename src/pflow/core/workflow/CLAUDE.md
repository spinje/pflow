...
├── status.py                # WorkflowStatus enum: SUCCESS/DEGRADED/FAILED/DENIED/PAUSED
...
9. Node-specific static parameter semantics — per-node-type param checks (e.g. agent backend/parameter and structured-output schema preflight).
...
Five states: `SUCCESS` (all nodes clean), `DEGRADED` (completed with warnings, e.g., unresolved templates in permissive mode), `FAILED` (errors), `DENIED` (a human denied an approval gate — Task 125; exit 3, never error-routed), `PAUSED` (a gate fired non-interactively with tracing on and the run is durably waiting for an answer — Task 171; exit 4, `success=False`, resume token = `execution_id`).
