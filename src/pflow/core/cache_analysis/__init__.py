...
(``1.0`` → ``1.1``) are additive (new fields, new warning IDs); consumers using
``format_version.startswith("1.")`` keep working. Major bumps (``1.x`` →
``2.x``) are breaking. Mirrors the trace ``2.x`` consumer policy at
``trace_report.py``.
"""
