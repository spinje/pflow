...
**Cross-boundary recommendations (`cache.sub-workflow-cache-undeclared`) take a different shape — emit + warn, never suppress.** When the parent value's estimated tokens are below the child model's minimum cache threshold, the recommendation still fires (with `savings_usd=None`) but its body message gains a `Note: ~N tokens estimated, below {model}'s {threshold}-token minimum` clause. Different from the greenfield suppression because the cross-boundary path's recommendation surface (`Recommended actions`) is the agent's actionable signal — suppressing would hide the boundary from the agent who needs to know about it. The warning prose carries the "declare AND increase content" framing. Heterogeneous-children case (different child models with different thresholds) samples the first child's model only; mirrors the existing single-model sampling in `_consolidate_to_root_advisories`.
