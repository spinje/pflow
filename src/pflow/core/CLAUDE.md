...
| LLM adapter — deterministic provider error (4xx that retry won't fix) | `LLMCallError` subclass — `UnknownModelError(reason)`, `MissingApiKeyError(kind)`, `InvalidRequestError`, or `LLMResponseParseError` | structured `model`, `reason`/`kind`, `provider_message` (upstream text, key material masked at capture — distinct from the pflow-wrapped `Diagnostic.message`); each subclass overrides `to_diagnostics()` to produce rich Diagnostics. LLMNode catches at `_call_llm` (preventing retry burn) and lifts `_diagnostic_context` into shared. |
...
**Explicit `api_key` pass**: for pflow's registered providers, `complete()` resolves the key via `llm_config.resolve_provider_api_key` (canonical-first) and passes it as `api_key=`, unless the caller supplied one via `model_options`. Load-bearing: LiteLLM's own env lookup uses a different precedence (Gemini: `GOOGLE_API_KEY` before `GEMINI_API_KEY`), so delegation let a stray `GOOGLE_API_KEY` silently shadow the key validation had checked — validate-green workflow, provider 404 at runtime. Unregistered prefixes (`openrouter/...`) keep full delegation.

**`provider_message` is masked before capture**: `_classify_litellm_error` runs `_mask_key_material` over the raw exception text (Google echoes the API key as a `?key=` URL param in some errors) because `provider_message` renders to the terminal (`diagnostic_render` "Provider response:" block, capped at 5×300 chars; batch item errors render it via `batch_error_details["provider_message"]`) and lands in traces/JSON. The exact key `complete()` put on the wire is passed as `secret=` and replaced verbatim (covers arbitrary key shapes); labelled (`key=`/`api_key=`) and `AIza…`/`sk-…` patterns are the fallback. The `LLMResponseParseError` site in `complete()` is the one construction outside the classifier — it masks through the same helper. Don't remove the masking as "unused" — it guards an exposure, not a code path.
...
**Key sources**: `resolve_provider_api_key(provider)` is the single resolution algorithm — walks the provider's env vars canonical-first (`GEMINI_API_KEY` before `GOOGLE_API_KEY`), `os.environ` then `~/.pflow/settings.json` env section. `_has_provider_key` derives from it, and `llm_client.complete()` passes its result explicitly to LiteLLM — so presence-check and the key a call actually uses can never disagree. `inject_settings_env_vars()` (called early in CLI/MCP startup) still pushes settings-stored keys into the environment for anything outside the adapter's registered providers. User's actual environment takes priority over settings (won't override existing vars). Idempotent. Skipped under `PYTEST_CURRENT_TEST`.
