...
- **Format 2.1.0** (Task 159): Tree-structured events with `node_output`, `template_resolutions`, `node_params`, `batch_items`, `sub_workflow_events`. 2.1.0 adds top-level `workflow_path` (resolved file path or `ir-hash:<md5>` for inline runs — symmetric with `MemoizationCache.workflow_path` scoping) plus per-event `cache_key` / `cache_source` (`"memo" | "in_process"`) / `cache_age_sec` / `cache_chunks_skipped` flowing through `event["llm_call"]` via the existing `llm_usage` channel. The `format_version.startswith("2.")` consumer gate keeps 2.0.0 readers compatible (new fields are additive). Cache-metadata fields are gated by `_should_write_cache_metadata(node_type_name)` — currently allowlisted to `LLMNode` only; ClaudeCodeNode is INTENTIONALLY excluded (its cache tokens come from the SDK, a different cache layer).
