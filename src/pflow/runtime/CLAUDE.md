...
- Per-event `event["node_id"]` and `event["llm_call"]["model"]` are consumed by `prompt_cache_analysis.analyze._trace_aligns_with_ir` for auto-load drift detection. The existing trace shape is sufficient; no producer-side fingerprint or format bump is required for that consumer.
