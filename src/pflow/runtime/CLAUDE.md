...
- Per-event `event["node_id"]` and `event["llm_call"]["model"]` are consumed by `prompt_cache_analysis.trace_loading` and `prompt_cache_analysis.analyze._detect_per_node_model_drift` for trace autoload/listing and model-drift notes. The existing trace shape is sufficient; no producer-side fingerprint or format bump is required for those consumers.
