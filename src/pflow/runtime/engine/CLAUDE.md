...
3. **Full-run walk**: resets `node_visit_counts`, then `_execute_node` per node, routes via the shared kernel `route_action(action, successors)` (module-level — the same precedence rule the planner's `_classify` dispatches on: successor match > clean termination > routing error). `_run_node_with_child_only` is passed `None` (dormant — the dotted-child plumbing is kept for the deferred nested-targeting follow-up; `_pflow_child_only_node` is never written).
