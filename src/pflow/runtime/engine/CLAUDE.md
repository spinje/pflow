...
FLAT `--only` only (dotted rejected upstream). Instead of re-walking — which re-executes and re-fires side-effecting upstream on every iteration — restore every node's output from the most recent full successful run's trace and execute ONLY the target:
...
2. `seed_snapshot_into_shared(shared, events, exclude=this_only)` seeds `shared[node_id]` for every node EXCEPT the target (filtering the `apply_memo_hit` reserved keys). Restored nodes are recorded in `__execution__["restored_nodes"]`.
