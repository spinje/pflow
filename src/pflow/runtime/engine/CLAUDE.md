...
2. `_run_inner`: parses + validates `--only` via the shared `validate_only_target` (module-level, used identically by the planner — rejects unknown ids, empty string, AND dotted/nested targets, membership-checked first). **`--only` is snapshot semantics (issue #443), NOT a graph walk** — if `this_only` is set, `_run_only_snapshot` returns early. The walk below is full-run-only.
3. **Full-run walk**: resets `node_visit_counts`, picks the walk entry via `_walk_entry` (`start_node`, or the failed node K when `resume_from` is set — Task 164: `_prepare_resume` seeds upstream from the source trace's events via the shared `seed_walk_entry`, stamps `restored_nodes`/`resumed_from`/`resume_entry_node`, and re-records each restored node's final event as `cached=True, restored=True` so the attempt trace is self-contained; the walk itself is unchanged — one loop, parameterized entry), then `_execute_node` per node, routes via the shared kernel `route_action(action, successors)` (module-level — the same precedence rule the planner's `_classify` dispatches on: successor match > clean termination > routing error). `_run_node_with_child_only` is passed `None` (dormant — the dotted-child plumbing is kept for the deferred nested-targeting follow-up; `_pflow_child_only_node` is never written).
4. On `RouteKind.ROUTING_ERROR`: `last_action = _handle_no_successor(...)` (the assignment is load-bearing — it returns `"error"` so the run's action string reflects the failure). `_handle_no_successor` checks if step 17.5 already archived the node; if so, returns `'error'` without writing any hint (the real failure data is already in `__failures__`, and a generic "add on-error" hint would visually outrank the real fix — GH #437). Otherwise (a custom non-error action with no matching edge — a genuine routing bug) rolls back success bookkeeping, archives as `routing_error` via `mark_node_failed`, AND calls `self.trace.mark_last_event_failed(node_id, error=...)` to flip the already-recorded trace event so trace state agrees with `__failures__` (GH #250). The trace-flip call happens only in the non-error-action branch — the error-action branch is already correct because `is_error_action=True` caused step 16 to record `success=False`. On `CLEAN_STOP` the walk breaks without calling it (its internal `is_clean_termination` check is a defensive guard for future direct callers).
...
2. `seed_walk_entry(shared, events, entry=this_only, start_node=workflow.start_node)` — the shared module-level seed + locate-entry composition (Task 164 Phase 0; also called by the planner's `_resolve_walk_start`, parity pinned by `test_engine_and_planner_walk_entry_state_match`). It calls `seed_snapshot_into_shared(shared, events, exclude=this_only)` — seeding `shared[node_id]` for every node that ran BEFORE the target in the snapshot's execution order (filtering the `apply_memo_hit` reserved keys); templates can only reference earlier steps, so this is every node the target could read; DOWNSTREAM nodes are NOT seeded (their stale output must not be addressable), and neither are FAILED-final-status nodes (seed fidelity, Task 164 review fix — their data lived in `__failures__`, never the store) — and returns the entry node via `find_node_by_id` (BFS, cycle-guarded). Everything else stays caller-side by design (scope guard: no mode flags/callbacks). Restored nodes are recorded in `__execution__["restored_nodes"]`.
3. If `source_status == "degraded"`, `_emit_snapshot_degraded_advisory` writes a WARNING `Diagnostic` (id `only.snapshot-degraded`, built by the shared `build_snapshot_degraded_diagnostic` — the planner appends the same diagnostic with `source="planner"` and future-conditional tense) under `__warnings__["__only_snapshot__"]` → DEGRADED + loud advisory (a degraded run can carry partial upstream data; never seed it silently).
4. `_execute_node` runs the entry node (located in step 2) inside `loop_runtime_scope` (a `loop:` target runs ONE iteration). `_execute_node` step 2 still writes `__execution__["only_node"]`, so output routing stays target-scoped.
...
- **Allowlist semantics in `_should_write_cache_metadata`**: `LLMNode` only. `AgentNode` is INTENTIONALLY excluded — its `cache_creation_input_tokens` / `cache_read_input_tokens` come from the selected backend and reflect provider-side context caching (a different cache layer). Mixing pflow's memo `cache_key` / `cache_source` with backend cache tokens would mislead agents reading the trace. Adding a new LLM-producing node type that participates in pflow's memo cache requires extending this gate alongside the new node type's `post()` implementation.
...
### `execute_batch(node, config, shared, execute_single_fn) → str`

Returns just the aggregated action string. The `execute_single_fn` callback
signature is `(node, config, item_shared) → (action, last_resolutions,
template_errors)`. This is `engine._execute_single_node`.

**Per-item trace events stay in the shared store** at
`shared["_batch_trace"][node_id]`. `execute_batch` does NOT drain them. The
engine drains via `_collect_batch_trace(shared, config.node_id)` at exactly
one site per branch: right before `record_trace` on the success path, and in
the except handler on the failure path. The shared store is the recovery
channel — when `execute_batch` raises (fail_fast with errors), items that
completed before the failure survive into the trace because the same buffer
is what the except handler reads (Bundle 8 fix).
...
5. `_push_batch_warnings` — write to `__warnings__` for DEGRADED status
6. **Then** `execute_batch` raises if fail_fast had errors, or `_aggregate_batch_results` raised at step 4 for all-failed continue mode. The raise always happens AFTER `shared[node_id]` is populated.

The engine drains `shared["_batch_trace"]` after this returns (success) or in
the except handler (failure). Never drain inside `execute_batch` — that would
re-introduce the Bundle 8 bug class where a return-tuple gets discarded on
raise.

Batch error records keep `errors[].item` as the full original runtime input. `errors[].item_summary` is the display-safe companion generated by `batch_item_summary.summarize_batch_item()`. Any terminal, MCP, JSON/API, or report renderer should use the summary and must not print the raw item.

`errors[].provider_message` (optional) carries the raw upstream provider diagnosis so a batched LLM failure keeps the line that names the real cause. `_build_batch_error` fills it from **two** producers and both are needed: the raised exception's `provider_message` attribute, AND — the easy one to miss — an explicit value read out of the item's error-shaped result via `_extract_provider_message`, because `LLMNode` converts deterministic `LLMCallError`s into an error *dict* (`_diagnostic_context`) rather than raising, which is how a batched LLM failure normally arrives.
...
`{results, count, success_count, error_count, errors, batch_metadata}` — built by the shared `build_batch_output()` (single source for the 6-key contract + 6-key `batch_metadata`; the dry-run planner's `_build_batch_output_shape` delegates to it with `errors=[]`, `timing_stats=None`, so the plan-time shape cannot drift from runtime). `results` contains only successful items (failed items filtered out via `error_indices`). `errors` is the authoritative failure record with index, item, and error message per failure — **always a list (`[]` when there are no failures, never `None`)**, matching its declared `type: array`. `count` = total items attempted. `success_count` = `len(results)`. `error_count` = `len(errors)`. Verified by `BATCH_OUTPUTS` contract in `template_validation/validator.py` and pinned directly by `tests/test_runtime/test_batch_output_shape.py`.
...
- `handle_api_warning(...)` — records trace + metrics, fires the `node_warning` AND `node_complete` progress callbacks (the latter added for GH #249 so progress UIs stop showing the node as running), then archives via `mark_node_failed` at the END (so all bookkeeping reads `shared[id]` first). Uses `mark_node_failed` for the warning + failed_node writes, no direct writes.
...
- **Batch fail_fast raises in `execute_batch`, NOT in `_execute_sequential`/`_execute_parallel`.** The inner functions break out of their loops on first failure and return partial state. The raise happens AFTER `_aggregate_batch_results` writes `shared[node_id]` so step 17.5 can archive the metadata. Per-item trace events sit in `shared["_batch_trace"][node_id]` for the engine's drain on whichever branch (success or except) wins.
- **`execute_batch` never drains its own trace buffer.** The engine drains via `_collect_batch_trace` at exactly one site per branch (right before `record_trace` on success, in the except handler on failure). Moving the drain inside `execute_batch` or back to right-after-call would re-introduce Bundle 8's Bug B (items lost when a later step raises).
