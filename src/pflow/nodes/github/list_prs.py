"""GitHub pull requests listing node implementation."""
...
class ListPrsNode(Node):
    ...
    - Writes: shared["prs"]: list[dict]  # Array of PR objects
        ...
        - author: dict  # PR author information
            - login: str  # Username
            - is_bot: bool  # Whether author is a bot
        - labels: list[dict]  # PR labels
            - name: str  # Label name
        - headRefName: str  # Source branch name
        - baseRefName: str  # Target branch name
    ...
    name = "github-list-prs"  # CRITICAL: Required for registry discovery

    def __init__(self, max_retries: int = 3, wait: float = 1.0):
        """Initialize the GitHub list PRs node with retry support."""
    ...
    def exec(self, prep_res: dict[str, Any]) -> dict[str, Any]:
        ...
        # Build command with all necessary fields for PRs
        cmd = [
            "gh",
            "pr",
            "list",
            "--json",
            "number,title,state,author,labels,headRefName,baseRefName,isDraft,url,createdAt,updatedAt",
        ]
    ...
    def post(self, shared: dict[str, Any], prep_res: dict[str, Any], exec_res: dict[str, Any]) -> str:
        ...
        # Store the complete PRs list with native field names
    ...
    def exec_fallback(self, prep_res: dict[str, Any], exc: Exception) -> None:
        ...
        error_msg = str(exc)
        # Also check stderr for subprocess errors
        if hasattr(exc, "stderr") and exc.stderr:
            error_msg = f"{error_msg} {exc.stderr}"
        ...
        if "gh: command not found" in error_msg:
        ...
        elif "could not resolve to a repository" in error_msg.lower() or "repository not found" in error_msg.lower():
            ...
            raise ValueError(
                f"Failed to list pull requests after {self.max_retries} attempts. "
