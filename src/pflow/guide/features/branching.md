...
- **`carry:` feeds state from round N output to round N+1 input.** Keys are body input names; values must reference this loop node's latest output. The node's normal `inputs:` mapping is the round-1 seed. From round 2 onward, `carry:` overrides only the carried keys; non-carried inputs stay constant. Use a plain `${loop-node.output}` reference — **avoid a literal coalesce fallback** like `${loop-node.x ?? 0}` in `carry:`: if the body omits the output on a round, the literal resolves silently and re-seeds the carried value instead of failing, hiding the bug (validation warns about this). The round-1 default belongs in `inputs:`, not in a carry fallback.
