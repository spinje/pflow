...
`${node.result}` is always present: the parsed schema value on success, raw model text (a string) on a soft-failure. With an object schema (the usual case, shown below) success yields a dict, so an `isinstance(result, str)` check cleanly detects the soft-failure. `${node._schema_error}` is set ONLY on a soft-failure — read it inside the fallback branch (where it's guaranteed present), not as the branch condition. Do NOT write `${node._schema_error ?? ""}` as the branch input: when the call succeeded the field is absent, and a coalesce whose node ran but whose field is missing does not fall through to the literal — it stays unresolved. (If your schema's top-level type is itself `string`, the `isinstance` discriminator can't tell success from soft-failure — wrap the value in an object schema so the success shape is a dict.)
