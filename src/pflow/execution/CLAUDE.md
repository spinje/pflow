...
- **`--only` snapshot parity (issue #443)**: for a flat `--only` target, `_resolve_walk_start` seeds upstream from the most recent full run's trace (`load_snapshot_or_raise` + `seed_snapshot_into_shared`, same helpers the engine uses) and starts the walk AT the target via `find_node_by_id` — so the plan is a SINGLE entry (upstream/downstream not costed) and the target's templated params resolve against frozen upstream, making its cache verdict match the engine actually serving/executing it. No snapshot → `OnlySnapshotMissingError` (the loader's falsy `workflow_path` guard covers inline-without-path). `_validate_only_target` rejects dotted/nested targets with the SAME error category as the engine (membership checked first), so `run` and `plan` reject identical `--only` values identically. Pinned by `tests/test_runtime/test_only_snapshot.py` (verdict-vs-behavior parity) and `tests/test_runtime/test_dotted_only_path.py` (cross-entry rejection).
