...
- `format_only_indicator(only_node, nodes_skipped)` — `--only` mode confirmation line. Called from CLI default summary (`_display_execution_summary`), CLI `-p` mode emission (`_emit_mode_indicators`), and MCP text (`_append_execution_steps`).
- `format_resume_indicator(resumed_from, entry_node, nodes_restored)` — resume mode confirmation line (Task 164, `⤷ Resumed from <id> at '<K>' — N upstream steps restored`). Same three call sites as `format_only_indicator`; keyed off the `execution` dict's `resumed_from`/`resume_entry_node`/`nodes_restored` fields, which `format_execution_success` populates from the engine's `__execution__` stamps (absent on non-resumed runs).
