...
| `execution.wait` | Float ≥ 0, retry delay in seconds | Used by `Node` retry logic (`src/pflow/core/node.py`) |
...
> fields from IR. The built-in retry (`Node(max_retries=N)` in `core/node.py`) works independently.

**Interface Resolution:**

- Validation resolves inputs/outputs from node metadata
- Nodes may inherit from either BaseNode or Node from `pflow.core.node`
