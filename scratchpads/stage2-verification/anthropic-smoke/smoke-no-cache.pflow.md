# Anthropic Cache Smoke (no `## Cache` — control)

Two LLM calls with the same prompts as `smoke-with-cache.pflow.md` but
no `## Cache` declaration. Used as the baseline for cost comparison.
