...
- **Drift-detection-for-free seams** (raise recommendation strength for candidates touching them): the IR schema (`core/ir_schema.py`), the GraphModel (ADR-0003/0004 — structural `NodeId`, primitive-only), the litellm seam, and `tests/test_docs/` — its tests validate eligible shipped examples and complete guide workflow fences, with a self-contained guide subset also executed; see the collectors for exclusions and MCP skips.
