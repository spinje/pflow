...
description: "Implements exactly the assigned phase(s) of a task's implementation-plan.md in the task's worktree — tests as it goes, logs substance, stops on ambiguity. Model per launch follows DECISIONS #3/#8 (active override first; UI routing by taste/design needs). Never spawns agents."
...
- **User-facing phases: "verified" means you exercised the real surface** — run a real workflow
  ...
  skill is unavailable). Your model follows `.taskmaster/orchestration/DECISIONS.md` #3/#8
  (active override first; underlying UI routing by taste/design needs). Visual quality and UX
...
- **The progress-log entry is where the substance lives; the handback is a minimal pointer.**
  ...
  (format in `.taskmaster/orchestration/ORCHESTRATION.md` → Progress-log entry format).
  The orchestrator reads your log entry, not your diff — the
...
- **The two self-checks** (run in your live context when the orchestrator asks): (1) *"Are you
  ...
  ones, log which. Log it RESOLVED — `test-reflect (directed): …` or `not needed — <reason>`,
  never a parked "to run when directed".
- **When the orchestrator hands you the completion gate** (you built the phases and your window
  is healthy): follow `.claude/skills/deep-review/SKILL.md` — dispatch the selected lenses via
  its pflow fan-out, backgrounded to a declared output file, and WAIT IN-TURN for the merged
  report (Monitor until-loop or foreground polls; never end your turn on it — a foreground call
  past the 600s cap auto-backgrounds), verify Critical findings against code, apply the correct
  fixes, and log EVERY finding
  with disposition — fixed, or skipped with a reason. An empty or partial report is a coverage
  gap, not a clean pass — re-run the missing lenses. Hand the outcome back; the orchestrator
  dispositions what remains. You are preferred for this over a fresh agent precisely because you
  still hold the code in context; say so if your window has degraded far enough that you
  shouldn't.
