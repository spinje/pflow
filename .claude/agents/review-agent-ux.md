...
Before flagging a UX issue, check if infrastructure already exists. New code should use existing patterns. **Canonical reference: `src/pflow/core/CLAUDE.md`** — see its `exceptions.py` section for diagnostic guidance, then the class definitions in `core/exceptions.py` and `core/user_errors.py` for the appropriate structured exception.
...
| Validation warnings | Returned `Diagnostic` values | `print()` or `click.echo()` directly |
| Runtime warnings | `shared["__warnings__"]` dict keyed by node ID | `print()` or `click.echo()` directly |
