...
Historical duplication patterns to watch for recurring (several have since been consolidated; verify current consumers rather than treating this table as an open-bug list).
...
**`unittest.mock.patch()` strings** — stale patch targets may fail to resolve, or may still exist but no longer be the binding used by production, leaving the real call unmocked. Verify both target existence and the name looked up at the call site:
```
grep "patch.*pflow" tests/                  # All patch strings
# Check every patch target against actual module paths
```
Historical example: 53 string-based patch targets needed updating after module moves in Task 92.
...
**`patch()` strings after moves/renames** — stale or unused patch bindings (Task 92: 53 targets needed updating)
