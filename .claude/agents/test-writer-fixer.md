...
Resolution order: exact `model+schema` match → wildcard `"*" + schema` → built-in schema defaults → fallback `{"response": "mock response"}`.
...
- `write_workflow_file()` converts IR dicts to `.pflow.md` format. `ir_to_markdown` does NOT emit `edges`, `start_node`, or `ir_version` — execution order is inferred from step order.
