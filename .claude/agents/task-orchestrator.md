...
description: Runs ONE pflow task end to end in its worktree — plans (or implements a task-planner's plan), delegates phases, gates quality, writes task-review.md, opens the PR. Launched only by the main orchestrator. Opus intent by default; Fable opt-in subject to the active routing override.
model: opus
effort: high
---
...
4. **Execute**: delegate to `task-phase-implementer` agents per the plan's agent assignments.
   ...
   — and NEVER two engine-touching or trace-format-touching phases in parallel. **UI phases
   (`web/`) go to a specialist implementer — never implement UI inline. Apply DECISIONS #3's
   active override before #8's taste/design routing rule; never route below the applicable tier.**
...
6. **Completion gate** — only when you are FULLY happy (ask yourself first: all phases verified,
   ...
   implementer does not launch agents), launch it yourself when you commission the gate and hand
   its report to the gate-runner for evaluation alongside the fan-out's. (When you implemented
   directly as a small task, you are the builder — run your own gate.) Trace format touched? Run the Task-159 baseline
   (`.taskmaster/tasks/task_159/baseline/verify.sh`) as the outer regression net.
...
- **In Claude, a monitor you arm cannot wake you, and your implementers' handbacks reach the
  MAIN orchestrator, not you** (phase implementers have no send tool). Stopping after a dispatch
  is fine — the main orchestrator relays each handback with ground-truth — but every stop must
  name its exact resume condition, because that relay is what resumes you. **In Codex, receive
  and check your children's handbacks through the collaboration tools before continuing.**
