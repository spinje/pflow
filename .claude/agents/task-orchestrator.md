...
**Three entry modes** (your packet says which):
...
- **Issue mode** (lane B — the packet names a GitHub issue, not a task folder): the issue is your
  spec; no task artifacts (no plan file, no `/create-task-review`). Implement directly — tests,
  the Definition of done, and UI routing (DECISIONS #8) still govern; the completion gate scales
  per the `/deep-review` skill's tier table (trivial diffs may skip). Then `/create-pr` with a
  `Closes #N` body, and after CI green **you squash-merge your own PR** — the one exception to
  "you never merge". Handback: "merged — PR #<n>".
...
5. **Per-phase self-checks** (not a conformance diff-audit): read the implementer's progress-log
   ...
   **Only when a phase's tests were plausibly hard to write / easy to cheat** (subtle logic,
   ...
   scaffolding/config phases. After an especially risky phase (engine contact, trace-format
   change, resume/gate semantics later phases build on), run a mid-task review: 1–2 targeted
   specialists on that phase's diff. Gate green + self-checks done + deviations logged → move on.
6. **Completion gate** — only when you are FULLY happy (ask yourself first: all phases verified,
   ...
   `screenshot-pflow-web-ui` skill — green unit tests alone don't close user-facing work): run
   the **code-mode `/deep-review`
   battery on your full branch diff** (the skill's rubric picks the specialists). Verify Critical
   findings against code before accepting. **Apply the correct fixes** (yourself or via an
   implementer); log EVERY finding with disposition — fixed, or skipped with a reason. Trace
   format touched? Run the Task-159 baseline (`task_159/baseline/verify.sh`) as the outer
   regression net.
...
resume point (disaster recovery). You will be RESUMED via SendMessage with the ruling — never
...
- Never commit to `main`; push only via `/create-pr` at close-out; never merge (sole exception:
