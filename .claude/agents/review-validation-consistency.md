...
10-step pipeline that runs BEFORE compilation; emits `Diagnostic` objects natively. Canonical step list: `core/workflow/validator.py::WorkflowValidator.validate` — read it rather than trusting a summary. Shape: structural → stdin → stdout → data flow (cache rules live HERE, in `_validate_cache_block`) → templates (in `runtime/template_validation/`) → node types → output sources → unknown params → node-specific semantics → sub-workflows (recursive). A step-1 `Severity.ERROR` short-circuits everything after it; an unnumbered reserved-literal-name guard runs between steps 1 and 2. Step order is load-bearing (templates before node types — see that function).
...
**Key insight**: The runner orchestrates `WorkflowValidator` before compilation checks; it is not a third downstream validation stage. File references can already be resolved before validation, while runtime template values generally are not. Trace the actual data state at each boundary; Task 129 was a historical file-resolution ordering failure.
...
**Current boundary**: Template validation registers batch variables (`${item}`, `${__index__}`) in its local validation metadata. Runtime batch execution independently injects the actual item alias and index into each item store before template resolution. Skipping validation can lose coverage; it does not by itself omit runtime batch-variable initialization.
...
| `runtime/engine/batch_executor.py` | `core/workflow/data_flow.py` (batch-variable scope/reference validation), `runtime/template_validation/validator.py` + `batch_item_validation.py` (batch output shapes) |
...
```
rg 'startswith.*\$\{' src/pflow/          # Manual template detection
rg '\[2:-1\]' src/pflow/                 # Manual ${...} stripping
rg 'resolve_value' src/pflow/              # Low-level resolution bypassing resolver
```
...
| CLI `pflow probe` | No workflow validator/compiler — `execute_single_node` calls `node.run()` directly, with the two-phase `ExecutionCache` (not the memoization cache) | `cli/commands/_probe_impl.py` |
| MCP `registry_run` | Synthetic single-node IR through `WorkflowRunner.run()` and its validation/compilation path; memo cache and trace disabled | `mcp_server/services/execution_service.py::run_registry_node` |
...
- Registers `inputs` keys globally (not per-node) in validation output/context metadata
...
- If two nodes define different `inputs` dicts with overlapping keys, does the global registration in `template_validation/` miss cross-node misuse?
...
- Child declared outputs need registration in template validation for downstream references; runtime otherwise falls back to filtered child-store exposure
...
### 7. Known Boundaries and Gaps

These boundaries and gaps exist today. If changes touch these areas, check whether they change the boundary or worsen a gap; do not report a pre-existing gap as new:
...
- **The CLI probe path lacking workflow validation** — by design; it runs `node.run()` directly for fast iteration. Flag only if a change routes real workflow execution through it. MCP `registry_run` instead uses the runner validation/compilation path.
