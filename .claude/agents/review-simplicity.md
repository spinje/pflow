...
- **Indirection with a recorded reason** — the litellm lazy-import seam exists for CLI startup time; a seam with two real adapters is earning its keep. Run the canonical deletion test before flagging an abstraction: if removing it only spreads its complexity into callers, it is earning its keep; flag it when removal eliminates needless concepts or indirection.
