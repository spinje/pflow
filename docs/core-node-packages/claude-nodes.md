...
This is an intentional exception to our [simple node philosophy](../simple-nodes.md) - `claude-code` is a "super node" that handles complex development tasks that would require dozens of individual simple nodes. The comprehensive nature and tool integration of Claude Code CLI justifies this architectural decision.
...
```python
class ClaudeCodeNode(Node):  # Inherits from pocketflow.Node
    """AI-assisted development using Claude Code CLI with comprehensive project context.
    ...
    def exec(self, prep_res):
        # Execute Claude Code CLI in headless mode with comprehensive instructions
        cmd = [
            "claude", "--headless",
        ...
        return {
            "code_report": result.stdout,
            "files_modified": self._extract_modified_files(result.stdout),
            "analysis": self._extract_analysis_section(result.stdout),
            "implementation": self._extract_implementation_section(result.stdout),
            "summary": self._extract_summary(result.stdout)
        }

    def post(self, shared, prep_res, exec_res):
        shared["code_report"] = exec_res["code_report"]
        shared["files_modified"] = exec_res["files_modified"]
        shared["analysis"] = exec_res["analysis"]
        shared["implementation"] = exec_res["implementation"]
        return "default"
```
...
The `claude-code` super node receives comprehensive instructions generated by the [planner](../planner.md) that combine multiple development tasks into a single, cohesive prompt. This approach provides several advantages:
...
The super node writes to comprehensive shared store keys following the [shared store pattern](../shared-store.md#natural-interfaces):
...
- **Design Philosophy**: [Simple Nodes Pattern](../simple-nodes.md) - Why this is an exception
...
- **Planning System**: [Planner](../planner.md) - How template-driven prompts are generated
- **Related Nodes**:
