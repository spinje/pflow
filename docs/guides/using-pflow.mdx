...
pflow workflows are markdown files (`.pflow.md`) — readable documents that double as executable pipelines. Your agent writes them, but you can open any workflow to see exactly what it does and why. Just like any `.md` file, it renders everywhere — GitHub, your editor, any markdown viewer. Your agent:
...
> Your agent: "pflow needs an API key for intelligent discovery. Run this command: `pflow settings set-env OPENAI_API_KEY 'your-key'`"
...
Chain workflows together using pipes:

```bash
pflow -p step1 | pflow -p step2 | pflow step3
```

See [Stdin input](/reference/cli/index#stdin-input) for how workflows receive piped data.
