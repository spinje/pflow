...
2. **Runs instantly if found** - No repeated workflow generation, no LLM costs, same reliable result
...
```bash
pflow probe mcp-slack-SEND_MESSAGE channel="general" text="Done!"
```
...
See [workflow commands](/reference/cli/list) for more options.
...
You describe what you want done. Your agent does the thinking. pflow handles repeated execution — same steps, same order, same data flow, every time. The workflows your agent builds become your personal library, ready to reuse or chain into larger workflows.
