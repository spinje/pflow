...
When workflows fail, pflow gives your agent what it needs to fix them: structured error data with available fields, types, and fix suggestions — not just error messages. Since your agent builds and fixes workflows, error messages are the primary interface — they say what's wrong, what's available instead, and how to fix it.
