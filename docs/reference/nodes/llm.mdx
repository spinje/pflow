...
The LLM node calls AI models using [Simon Willison's llm library](https://llm.datasette.io/). Use it when a step needs reasoning — summarization, classification, extraction, anything a Python expression can't handle. It supports multiple providers (OpenAI, Anthropic, Google, local models) through a unified interface.
