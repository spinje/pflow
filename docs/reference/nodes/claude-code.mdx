...
1. **Scalar type coercion** — If the agent returns a structurally-valid response but a scalar field has the wrong type (e.g., `"false"` string instead of `false` boolean), pflow coerces it to the declared type using canonical conversions:
   - Boolean: `"true"`/`"false"` (case-insensitive) → `True`/`False`
   - Integer/Number: Numeric strings (`"3"`, `"3.14"`) → `int`/`float`
   - String: Non-string scalars → `str()`

2. **Resume retry** — If the agent produces no structured output, returns uncoercible values, or returns a value outside a declared `enum`/`const`, pflow asks the same session to re-emit the output with an explicit schema prompt. The `schema_retries` parameter (default `1`) controls how many corrective attempts to make.
