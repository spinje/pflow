...
2. **Resume retry** — If the agent produces no structured output or returns uncoercible values, pflow asks the same session to re-emit the output with an explicit schema prompt. The `schema_retries` parameter (default `1`) controls how many corrective attempts to make.
