...
| `prompt` | str | Yes | - | Task sent to the agent (max 10,000 characters) |
...
| `use_api_key` | bool | No | `false` | Use `ANTHROPIC_API_KEY` and Anthropic Console billing instead of a Claude Pro/Max subscription |
...
Claude reports `input_token_accounting: split_cache_fields` and an SDK-reported API-equivalent `cost_usd`. Codex reports `total_includes_cache`, adds `reasoning_output_tokens`, and leaves `cost_usd` as `null` because the CLI does not expose per-run USD cost.
...
<Tabs>
  <Tab title="Claude subscription">
    ...
    This is the default. The node blanks `ANTHROPIC_API_KEY` for the Claude subprocess only, because Claude Code otherwise prefers an ambient key and bills Anthropic Console even when a Pro/Max subscription is logged in. Sibling `llm` nodes still see the original environment.
  ...
  <Tab title="Claude API key">
    ...
    This opts into Anthropic Console per-token billing. The flag fails closed on ambiguous values so a string such as `"false"` cannot accidentally enable billing.
  </Tab>
  <Tab title="Codex login">
    ...
    The node uses the CLI's existing ChatGPT login and Codex configuration. It does not accept an OpenAI API key parameter.
  </Tab>
</Tabs>
...
| Codex authentication failed | CLI login is missing or expired | Run `codex login` |
