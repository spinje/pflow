...
Claude reports `input_token_accounting: split_cache_fields` and an SDK-reported API-equivalent `cost_usd`. Codex reports `total_includes_cache` and adds `reasoning_output_tokens`. When a workflow declares `model`, pflow estimates the Codex call's API-equivalent `cost_usd` from its LiteLLM pricing catalog and the CLI's cache-aware token usage. The value remains `null` when `model` is omitted or LiteLLM has no pricing for it; pflow does not inspect private Codex session files to infer the CLI-configured model.
