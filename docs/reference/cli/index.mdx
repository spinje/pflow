...
The `--only` flag runs the workflow from the start through the named node, then stops — downstream nodes don't execute. Upstream nodes along the way are re-run unless they're cached: only `llm` nodes (and any node with `cache: true`) come from cache, so a side-effecting upstream node like `shell: git push` runs again on each `--only` invocation:

```bash
pflow ./workflow.pflow.md --only process-data
```

Without `-o`, `--only` streams the targeted node or sub-workflow result to stdout instead of the workflow's full-run outputs. Pass `-o <key>` when you need a specific named output.
...
Generate a [Mermaid](https://mermaid.js.org/) flowchart from a workflow. Shows the graph topology — nodes, edges, conditional branches, error routes, inputs, outputs — that's otherwise scattered across individual node directives in the `.pflow.md` file.
