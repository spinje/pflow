...
**Resume workflows from a failed node** — when a long workflow fails partway through, pick up from the failed step instead of re-running everything from the top. Paired with durable resume tokens so approval gates can pause and resume across process restarts and non-interactive runs.
