...
Each result contains `item` (the original input) plus the inner node's outputs. This makes it easy to correlate results with their inputs: `${node.results[0].item}`.
