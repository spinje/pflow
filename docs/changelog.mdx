...
<Update label="April 2026" description="Unreleased" tags={["Improvements", "Breaking changes"]}>
  ## LLM library swap: Simon Willison's `llm` → LiteLLM

  pflow's LLM node now calls models through [LiteLLM](https://docs.litellm.ai/) instead of Simon Willison's `llm` library. LiteLLM speaks to 100+ providers natively (OpenAI, Anthropic, Google, OpenRouter, Ollama, Mistral, Bedrock, Azure, vLLM, ...) — no plugin install required for additional providers. Cost reporting (`llm_usage.cost_usd` in traces and CLI output) now comes from LiteLLM's pricing data, which is more current and comprehensive than the per-model table pflow used to maintain.

  Existing workflows run identically. Response shape, error messages, retry semantics, and the `llm_usage` dict are all preserved.

  **Improvements**
  - Built-in support for OpenRouter, Ollama, Mistral, Bedrock, Vertex AI, vLLM, and 100+ more providers without plugin installation. Use the model with its provider prefix (e.g. `openrouter/anthropic/claude-sonnet-4-5`, `ollama/llama3.2`).
  - Cost reporting (`cost_usd` in traces and `pflow report`) is sourced from LiteLLM and stays current with provider pricing changes — no more `$0.00` for newly-released models.
  - Trace events now include `llm_prompt` for every literal-prompt LLM call, and `pflow report` renders a `## Prompt` section per LLM node — making it easier to audit what was actually sent to each model.
  - Deterministic LLM provider errors (unknown model, missing API key, schema rejection) now short-circuit at the adapter boundary instead of burning three retries — the CLI surfaces the friendly fix message in ~1 second instead of ~30. A bare model name with no provider prefix (e.g. `pflow run --model gibberish`) now produces the "use a provider prefix like `anthropic/...`" hint instead of a raw provider JSON envelope.
  - **Structured LLM error context in JSON output.** Failed LLM calls now expose `error_class`, `model`, and a discriminator (`reason` for unknown-model variants, `kind` for missing-key variants) in the `errors[i].context` block — agents can branch on cause programmatically without parsing prose. Diagnostic category is `"llm_failure"` (distinct from the generic `"execution_failure"`), so cost-gating, retry-gating, and key-rotation policies can filter LLM-specific failures.
  - **Empty-response warnings now surface in JSON output and shift the workflow status to DEGRADED.** Previously a reasoning model that spent its entire `max_tokens` budget on internal thinking (returning `response: ""`) silently looked like a successful run. The adapter now emits a structured warning (with discriminators for `length`/`max_tokens`/`content_filter`/`stop`/`null` finish reasons) that lands in `result["warnings"]` and the trace's `warnings` array, with dual remediation for reasoning-model traps ("increase max_tokens or lower reasoning_effort").
  - **Top-level cost tri-state.** When pricing data is unavailable for any LLM call (Ollama, custom endpoints, brand-new models), the success result now exposes `pricing_available: false`, `partial_cost_usd`, and `unavailable_models` at the top level alongside the bare `total_cost_usd: null` — agents can distinguish "no LLM calls" from "calls happened but pricing is missing" without drilling into the metrics block.

  **Breaking changes**
  - **API keys must come from environment variables or `pflow settings set-env`.** pflow no longer reads keys stored via `llm keys set <provider>` — Simon Willison's `llm` CLI subprocess is gone. If you previously stored keys with `llm keys set ...`, transfer them:
    ```bash
    ...
    ```
  - Plugin packages `llm`, `llm-anthropic`, and `llm-gemini` are no longer pflow dependencies. If you depended on them transitively, install them explicitly.

  <Note>
    Memo cache entries written before this upgrade lack `cost_usd`. Dry-run plans (`pflow ... --dry-run`) for cached nodes will show `estimated_cost_usd: null` and `cost_basis: upper_bound` until the 24-hour cache TTL refreshes them. New runs are unaffected.
  </Note>

  <Accordion title="What stays the same">
    - All existing `.pflow.md` workflow files run identically — model names, parameters, output shape, error messages.
    - The `cache: bool` per-node memoization opt-out is unchanged.
    - Image attachments, structured output (`output_schema`), reasoning (`reasoning_effort` / `reasoning_max_tokens`), and timeouts work the same way.
    - `pflow settings llm show` / `set-default` / `set-discovery` / `set-filtering` work the same way.
  </Accordion>
</Update>
