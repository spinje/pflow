    ...
    try:
        ...
        result5 = runner.invoke(
            ...
            [
                ...
                "--tool",
                "upper",
                json.dumps({"tool_calls": [{"name": "upper", "arguments": {"text": "one"}}]}),
            ],
        )
        ...
        assert (
            runner.invoke(
                ...
                [
                    "-c",
                    json.dumps({"tool_calls": [{"name": "upper", "arguments": {"text": "two"}}]}),
                ],
            ).exit_code
            == 0
        )
        ...
        assert (
            runner.invoke(
                cli.cli,
                ["chat", "-c"],
                input=(json.dumps({"tool_calls": [{"name": "upper", "arguments": {"text": "three"}}]}) + "\nquit\n"),
                catch_exceptions=False,
            ).exit_code
            == 0
        )
        ...
        result6 = runner.invoke(
            ...
            [
                ...
                "--tool",
                "output_as_json",
                json.dumps({"tool_calls": [{"name": "output_as_json", "arguments": {"text": "hi"}}]}),
                "--td",
            ],
        )
...
def test_register_toolbox(tmpdir, logs_db):
    ...
    conversation.chain(
        json.dumps({"tool_calls": [{"name": "Memory_get", "arguments": {"key": "hello"}}]}),
        after_call=after_call,
    ).text()
    assert accumulated == [
    ...
    try:
        ...
        assert result5.output == "Error: Toolbox tools (Filesystem) are not yet supported with llm -c\n"
