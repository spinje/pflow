    ...
    if json_:
        ...
        for toolbox in toolbox_objects:
            ...
            for method in toolbox.introspect_methods():
                sig = str(inspect.signature(method["implementation"])).replace("(self, ", "(").replace("(self)", "()")
                click.echo(
                    "  {}{}\n".format(
                        method["name"],
                        sig,
                ...
                if method["description"]:
                    click.echo(textwrap.indent(method["description"].strip(), "    ") + "\n")
...
def _gather_tools(tool_specs: List[str], python_tools: List[str]) -> List[Union[Tool, Type[Toolbox]]]:
    ...
    if bad_tools:
        ...
        bad_tool_classes = list(set(bad_tool.split("_")[0] for bad_tool in bad_tools if bad_tool[0].isupper()))
        if bad_tool_classes:
            raise click.ClickException("Toolbox tools ({}) are not yet supported with llm -c".format(", ".join(bad_tool_classes)))
        else:
