...
# AGENTS.md size — oversized docs signal oversized modules
wc -l src/pflow/<module>/AGENTS.md
```
...
- **AGENTS.md health**: Does each directory have one? Is it the right size? A 400-line AGENTS.md covering 19 files signals the directory needs splitting. No AGENTS.md at all means agents fly blind.
...
- **Subdirectory extraction** (cohesive file group → own directory with AGENTS.md)
...
- **Subdirectories**: "These 5 files form a cohesive group. Extract to a subdirectory with its own AGENTS.md?"
...
- **Documentation**: AGENTS.md references, comments
...
3. **Duplicate small helpers** — if a utility is <15 lines and only needed to avoid a cycle, copy it. Document the duplication in AGENTS.md. This is better than creating a `shared_utils.py` for one function.
...
# AGENTS.md files project-wide
grep -rn "old_module_name" --include="AGENTS.md" .
...
**Content placement principle**: Agents in subdirectories automatically see parent AGENTS.md files. So:
- **Subdirectory AGENTS.md**: per-file non-obvious details, internal dependencies, known issues, key lessons — content only relevant when working on those files
- **Parent AGENTS.md**: cross-cutting concerns (error philosophy, integration map, security issues), brief pointer to subdirectory AGENTS.md
- Don't duplicate between the two — the hierarchy gives agents both

**Checklist:**
- Update the parent module's AGENTS.md — remove moved-file details, replace with pointer
- Write a focused subdirectory AGENTS.md with the moved details
- If test files moved, add source-to-test mapping and any non-obvious test patterns
- Grep ALL other AGENTS.md files for references to moved/renamed modules — stale references in other directories are easy to miss
...
- A test-specific AGENTS.md would help future agents understand test patterns
...
- Write a short AGENTS.md with source-to-test mapping table and any non-obvious patterns (e.g., "each file creates its own mock registry — intentional isolation, don't extract to conftest")
