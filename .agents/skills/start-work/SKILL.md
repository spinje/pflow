...
If its a task id start by reading the .taskmaster/tasks/task_<task_id>/task-<task_id>.md file. If there are files in any subfolders list them to the user and ask if you should read those as well before starting to understand the task deeper.

If you receive a github issue number or a github link including the issue number, use gh cli to get the issue details, make sure to read all comments if any on the issue.
...
If a question can be answered by exploring the codebase, explore the codebase instead. Questions to the user and exploring the codebase using subagents can be done in parallel or interleaved if that makes more sense.
...
When the plan is agreed: ask the user whether to capture it — update the existing task spec, or
use the `create-task` workflow if no task file exists. **Never invoke `create-task` yourself — it
requires the user's explicit go-ahead.** For plans of significant scope, offer to run
`deep-review` (plan mode) on the captured plan before implementation begins — plan-stage findings
are the cheapest to fix. Once implementation begins, maintain the progress log with the
`create-progress-log` workflow.
...
- If something has been verified and the path forward is clear, ask for confirmation dont complicate the conversation with unnecessary questions unless tradeoffs are genuinely unclear or the decision has lasting implications that cant be easily reversed.
...
Do not read task-review.md or adr files yourself, let the subagents do it and explicitly ask them to only return with the information that is clearly relevant not a summary of what the files said. You should try to preserve your context window and explicitly ask for relevant information only by explaining the context clearly.
