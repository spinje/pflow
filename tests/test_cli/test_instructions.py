...
class TestCreateInstructions:
    ...
    def test_create_instructions_returns_full_content(self, runner: CliRunner) -> None:
        ...
        line_count = len(result.output.split("\n"))
        assert 1400 <= line_count <= 2400, f"Expected ~1800-2300 lines, got {line_count}"
    ...
    def test_create_instructions_part3(self, runner: CliRunner) -> None:
        ...
        # Part 3 should have ~900 lines (grew with conditional branching + dynamic routing examples)
        line_count = len(result.output.split("\n"))
        assert 400 <= line_count <= 1000, f"Expected ~900 lines for Part 3, got {line_count}"
