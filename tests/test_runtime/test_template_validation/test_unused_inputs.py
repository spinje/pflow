...
def test_unused_input_single_unused(mock_registry, tmp_path):
    ...
    assert len(errors) == 1
    assert "Declared input(s) never used as template variable: unused_param" in errors[0].message
...
def test_empty_inputs_field(mock_registry, tmp_path):
    ...
    errors, _warnings = split_template_diagnostics(workflow_ir, {}, mock_registry)
    assert len(errors) == 0
    ...
    errors, _warnings = split_template_diagnostics(workflow_ir_no_inputs, {}, mock_registry)
    assert len(errors) == 0
...
def test_multiple_unused_inputs(mock_registry, tmp_path):
    ...
    assert "unused1, unused2, unused3" in error_msg
...
def test_unused_input_with_missing_required_input(mock_registry):
    ...
    error_messages = " ".join(d.message for d in errors)
...
def test_case_sensitivity_in_unused_detection(mock_registry, tmp_path):
    ...
    assert len(errors) == 1
    assert "myinput" in errors[0].message
    assert "MyInput" not in errors[0].message  # MyInput is used
...
def test_mixed_nested_and_toplevel_templates(mock_registry):
    ...
    assert len(errors) == 1
    assert "unused_param" in errors[0].message
    # But the other nested ones should not be reported as unused
    assert "direct_url" not in errors[0].message
    assert "header_token" not in errors[0].message
    assert "body_message" not in errors[0].message
...
def test_partially_used_nested_inputs(mock_registry):
    ...
    assert len(errors) == 1
    assert "also_never_used" in errors[0].message
    assert "never_used" in errors[0].message
    # Used inputs should not be in the error
    assert "used_in_header" not in errors[0].message
    assert "used_in_body" not in errors[0].message
    assert "used_in_list" not in errors[0].message
