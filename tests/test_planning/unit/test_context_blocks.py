...
class TestImmutablePattern:
    ...
    def test_append_planning_block_returns_new_list(self):
        ...
        new_blocks = PlannerContextBuilder.append_planning_block(
            original_blocks,
            "Planning output text",
            {"status": "FEASIBLE", "node_chain": "node1 >> node2"}
        )
    ...
    def test_append_workflow_block_returns_new_list(self):
        ...
        new_blocks = PlannerContextBuilder.append_workflow_block(
            original_blocks,
            workflow,
            attempt_number=1
        )
...
class TestFourBlockLimit:
    ...
    def test_multiple_retries_exceeding_limit(self):
        ...
        with_plan = PlannerContextBuilder.append_planning_block(
            base_blocks,
            "Plan output",
            {"status": "FEASIBLE", "node_chain": "node1"}
        )
        
        # Add first workflow attempt (4)
        with_workflow_1 = PlannerContextBuilder.append_workflow_block(
            with_plan,
            {"attempt": 1},
            attempt_number=1
        )
        ...
        with_errors_1 = PlannerContextBuilder.append_errors_block(
            with_workflow_1,
            ["Error 1", "Error 2"]
        )
        ...
        with_workflow_2 = PlannerContextBuilder.append_workflow_block(
            with_errors_1,
            {"attempt": 2},
            attempt_number=2
        )
...
class TestEdgeCases:
    ...
    def test_max_retry_history_limit(self):
        ...
        for i in range(5):
            blocks = PlannerContextBuilder.append_workflow_block(
                blocks,
                {"attempt": i + 1},
                attempt_number=i + 1
            )
            blocks = PlannerContextBuilder.append_errors_block(
                blocks,
                [f"Error {i + 1}"]
            )
...
class TestContentStructure:
    ...
    def test_xml_tags_properly_formatted(self):
        ...
        assert dynamic_block.count("<user_request>") == dynamic_block.count("</user_request>"), "user_request tags paired"
        assert dynamic_block.count("<user_values>") == dynamic_block.count("</user_values>"), "user_values tags paired"
        assert dynamic_block.count("<requirements_analysis>") == dynamic_block.count("</requirements_analysis>"), "requirements tags paired"
        assert dynamic_block.count("<available_nodes>") == dynamic_block.count("</available_nodes>"), "nodes tags paired"
    ...
    def test_cache_control_markers_present(self):
        ...
        with_workflow = PlannerContextBuilder.append_workflow_block(
            with_plan,
            {"test": "workflow"},
            attempt_number=1
        )
...
**Node Chain**: github-list-issues >> llm"""
        with_plan = PlannerContextBuilder.append_planning_block(
            base,
            plan_output,
            {"status": "FEASIBLE", "node_chain": "github-list-issues >> llm"}
        )
        ...
        with_errors = PlannerContextBuilder.append_errors_block(
            with_workflow,
            ["Missing required parameter 'prompt' for llm node"]
        )
        ...
        final_blocks = PlannerContextBuilder.append_workflow_block(
            with_errors,
            workflow_v2,
            attempt_number=2
        )
