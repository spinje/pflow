...
MCP (and CLI) now receive validation warnings via ExecutionResult.validation_warnings.
...
1. Real warnings — a shell node's stdout is typed as str, so nested access like
   ${fetch.stdout.nested_field} triggers a ValidationWarning about JSON auto-parsing.
...
from pflow.runtime.template_validation.utils import ValidationWarning
...
class TestValidationWarningsPropagateToResult:
    """Validation warnings from WorkflowValidator reach ExecutionResult.validation_warnings."""
    ...
    def test_real_nested_str_access_produces_validation_warning(self):
        ...
        the validator emits a warning and it appears in result.validation_warnings.
        ...
        assert "${fetch.stdout.nested_field}" in warning["template"], (
            f"Warning template should contain the nested access, got: {warning['template']}"
        )
    ...
    def test_mocked_warnings_propagate_through_runner(self):
        ...
        assert len(result.validation_warnings) >= 1, (
            f"Expected at least 1 validation warning, got: {result.validation_warnings}"
        )

        # Find our synthetic warning by template
        matching = [w for w in result.validation_warnings if w.get("template") == "${api.result.data.items}"]
        assert len(matching) == 1, (
            f"Expected exactly 1 warning with our synthetic template, "
            f"got {len(matching)} in {result.validation_warnings}"
        )
