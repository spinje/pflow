...
def test_dynamic_cache_ttl_allowed_for_gemini() -> None:
    diagnostics = validate_data_flow(_cache_ttl_workflow(ttl="11m", model="gemini/gemini-2.5-flash"))
    assert "cache.unsupported-provider-ttl" not in {diag.id for diag in diagnostics}


def test_dynamic_cache_ttl_rejected_for_anthropic() -> None:
    diagnostics = validate_data_flow(_cache_ttl_workflow(ttl="11m", model="anthropic/claude-sonnet-4-5"))
    found = [diag for diag in diagnostics if diag.id == "cache.unsupported-provider-ttl"]
    assert found
    assert found[0].context["ttl_seconds"] == 660


def test_dynamic_cache_ttl_defers_for_templated_model() -> None:
    diagnostics = validate_data_flow(
        _cache_ttl_workflow(ttl="11m", model="${model}"),
        check_inputs=False,
    )
    assert "cache.unsupported-provider-ttl" not in {diag.id for diag in diagnostics}


def test_dynamic_cache_ttl_ignored_when_no_provider_cache_marker() -> None:
    diagnostics = validate_data_flow(
        _cache_ttl_workflow(ttl="11m", model="anthropic/claude-sonnet-4-5", prompt_cache=[]),
        check_inputs=False,
    )
    assert "cache.unsupported-provider-ttl" not in {diag.id for diag in diagnostics}
