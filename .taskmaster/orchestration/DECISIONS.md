...
screenshot-verification half stands regardless of tier).
...
codex — model-family diversity), in the FOREGROUND, so the whole gate is one job owned by the
gate-runner; direct Agent-tool lens launches are a logged one-off. Imported via the fold (#19;
source ledger rows #37 + #29).
