...
## Your Mission
Implement a planner-stage runtime validation loop with a new RuntimeValidationNode that executes the candidate workflow, classifies runtime issues, and triggers a targeted “runtime-fix” regeneration. Extend the HTTP node with deterministic extraction and structured errors. Support MCP tools by harvesting namespaced errors and missing downstream paths.
...
### Phase 1: Node and Error Foundations (2–3h)
...
2. Extend HTTP node in `src/pflow/nodes/http/http.py`:
   ...
   - Success → add `extracted` to result; Failure → raise `RuntimeValidationError`.
   - Ensure exec_fallback re-raises `RuntimeValidationError` unchanged.

### Phase 2: Planner Integration (2–3h)
...
3. MCP path test: simulate namespaced error or missing downstream path → `runtime_fix`.
4. Integration: planner E2E loop for HTTP guess→fix→success.
...
## Success Criteria
- ✅ All spec Test Criteria pass
- ✅ `make test` passes without regressions
- ✅ `make check` passes (lint, type, format)
- ✅ Planner flow shows new wiring and actions
- ✅ HTTP extraction behavior matches spec (keys, error formatting)
- ✅ MCP issues detected and classified per spec

## Common Pitfalls to Avoid
- DON’T add a `raw` return key to HTTP; keep `response` + `extracted` only.
- DON’T alter ValidatorNode behavior or planner compile order.
- DON’T mutate original execution_params or shared; use a fresh shared store in RuntimeValidationNode.exec().
...
## What NOT to Do
- DON’T add new CLI flags or planner config
- DON’T introduce third-party JSONPath libraries
- DON’T change existing node error contracts beyond HTTP additions
...
## Remember
This loop makes pflow learn external structures at generation time, then run deterministically afterwards. Your implementation unlocks “Plan Once, Run Forever” for real-world APIs and tools.
