...
- **`context: dict[str, Any] | None` for enrichment data**: HTTP status codes, shell stderr, MCP error details, available template fields are heterogeneous and node-type-specific. Typed fields would bloat the dataclass with 15+ optional fields each relevant to one node type. `context` is the Sentry pattern — clean structured message + optional context bag. Display code uses `message` + `suggestion` for warnings and most errors. JSON output includes `context` when present (agents get structured data for deeper parsing). Warnings usually have `context=None`, but narrowly scoped machine-useful fields are allowed when they improve actionability without affecting text rendering (for example `template` on validator warnings or unresolved-template details on runtime warnings).
