...
Step 5 of the merge prep — cleaning up the working scratchpads (throwaway by design; committed work persists, scratchpads do not). Anything load-bearing has already been lifted into durable artifacts: implementation chronology + WHY → this progress log; forward-looking backlog → followups doc; what shipped → GH issues #401–#404 + their PR #405 closure; bug-shape repros for the most-cited UX failure modes → inlined into the issue bodies that close them (PR #405 prep step). Manual repros were re-verified end-to-end against the shipped fixes during PR prep (see PR #405 test plan).

Note on historical scratchpad references in older entries of this progress log: lines that reference `scratchpads/...` paths in earlier 2026-05-14 / 2026-05-15 entries (Bundle 0 batch-UX, Bundle 8) are snapshot-in-time annotations of the verification commands at the moment of implementation. The assertions about what was verified remain accurate; the paths themselves will be stale after the scratchpads are cleaned up. The same shape-of-bug content has been preserved in the inlined repros on issues #402 / #403 / #404 / the #400 comment.

## 2026-05-18 — PR #405 pre-merge code review fixes

Three fixes from a multi-agent code review of the staged PR.

**C1 — `_PREDICTION_SKIPPED` sentinel leak in `cache.discrepancy` (critical).** `_predicted_key_for_event` returned the raw mapping value, so the sentinel `"__PREDICTION_SKIPPED__"` could surface verbatim in `predicted_cache_key` context AND drive a spurious `key_mismatch` attribution when paired with a real `actual_key`. Fixed at the helper boundary: filter the sentinel to `None` so the discrepancy gate AND the diagnostic context get clean semantics. `context.py:319` and `token_estimation.py:634` were already correct; this site was the last missed consumer of the Bundle 6 sentinel contract. Regression test `test_discrepancy_silent_when_prediction_skipped_sentinel_present` covers both failure modes.

**W1 — Batch trace drain symmetry (warning).** Bundle 8's success-path drain at `engine.py:576` popped the per-item buffer immediately after `execute_batch` returned. If a later step (`write_memo_cache`, `detect_api_warning`, metrics) raised, the except handler's drain popped an already-empty buffer and `record_trace` got empty `batch_trace_items` — items silently lost, the exact bug class Bundle 8 was meant to close. Moved the drain to one site per branch (right before `record_trace`); the except handler's drain stays unchanged. Added a defensive drain on the API-warning early-return path so the accumulator doesn't leak into shared state. Regression test `test_succeeded_batch_persists_items_when_post_exec_step_raises` drives real `WorkflowEngine.run()` with `write_memo_cache` patched to raise — mutation-verified.
