...
- workflow_metadata: Optional[dict] - Display/storage metadata (Path B only)
  ...
  # NOT used for parameter extraction (use workflow_ir["inputs"])
...
- missing_params: List[str] - Missing required parameters (only when incomplete)

Action strings:
- ParameterMappingNode returns "params_complete" when all required parameters found
- ParameterMappingNode returns "params_incomplete" when missing required parameters
...
# Note: Verification is implicit in routing - "params_complete" means success
...
- Confusion about workflow_metadata vs workflow_ir — Resolution: ParameterMappingNode uses workflow_ir["inputs"], not metadata
- verified_params structure seemed redundant — Resolution: Removed; routing action IS the verification result
...
- Chose routing actions over verified_params structure (simpler, clearer)
- Chose to clarify workflow_ir vs metadata distinction (prevents confusion)
