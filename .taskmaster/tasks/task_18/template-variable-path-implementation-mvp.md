...
1. **Template Variable Detection**
   ...
   - MUST support both `$variable` and `${variable}` syntax (Note: `$variable` is the standard format generated by the planner, `${variable}` supported for edge cases)
...
1. **Template Syntax**
   ...
   Note: The planner generates $identifier format exclusively.
   The ${identifier} format is supported for edge cases where
   disambiguation is needed (e.g., "${var}suffix" vs "$varsuffix").
   ```
...
4. **Error Handling**
   ...
   - **Missing variables**: Template remains unchanged for debugging (e.g., `$undefined` stays as `$undefined`)
...
Example flow:
- User provides: `--file_path=data.txt`
- Workflow has: `{"params": {"file_path": "$file_path"}}`
- Template resolver: Puts "data.txt" in node's params
- Node's prep(): `shared.get("file_path") or self.params.get("file_path")` gets "data.txt"
...
class TemplateAwareNodeWrapper:
    ...
    def __init__(self, inner_node, node_id: str, initial_params: Optional[Dict[str, Any]] = None):
        ...
        Args:
            ...
            node_id: Node identifier from IR (used for debugging/tracking)
            initial_params: CLI parameters for template resolution
        """
        self.inner_node = inner_node
        self.node_id = node_id  # Stored for debugging; may enable future namespacing features
...
def compile_ir_to_flow(
    ...
    Args:
        ...
        initial_params: CLI parameters for template resolution
    """
    # Note: initial_params comes from the planner's parameter extraction
    # Example: {"issue_number": "1234", "repo": "pflow"}
    # These are the values extracted from natural language by the planner
    flow = Flow()
    nodes = {}
...
def test_multiple_templates_in_string():
    """Test multiple template variables in a single string."""
    context = {
        "issue": {"number": 42},
        "user": {"name": "Alice"}
    }
    ...
    assert TemplateResolver.resolve_string(
        "Fix issue #$issue.number by $user.name",
        context
    ) == "Fix issue #42 by Alice"

def test_node_id_namespacing_pattern():
    """Test the node ID namespacing pattern from task 17."""
    context = {
        "github_main": {"issue_data": {"title": "Main bug"}},
        "github_fork": {"issue_data": {"title": "Fork bug"}}
    }

    assert TemplateResolver.resolve_string(
        "Main: $github_main.issue_data.title, Fork: $github_fork.issue_data.title",
        context
    ) == "Main: Main bug, Fork: Fork bug"

def test_empty_string_value():
    """Test template resolution with empty string values."""
    context = {"empty_var": "", "null_var": None}

    # Empty string should resolve to empty
    assert TemplateResolver.resolve_string("Value: [$empty_var]", context) == "Value: []"

    # None should convert to empty string per spec
    assert TemplateResolver.resolve_string("Value: [$null_var]", context) == "Value: []"

def test_template_in_multiline_string():
    """Test templates work in multiline strings."""
    context = {"title": "Bug Fix", "description": "Critical issue"}
